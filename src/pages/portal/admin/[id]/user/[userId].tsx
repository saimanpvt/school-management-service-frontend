...
const UserProfile = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await apiServices.users.update(
        userId as string,
        editData
      );
  ...
  const getRoleInfo = (role: number) => {
    switch (role) {
      case 1:
        return { name: 'Admin', icon: UserCheck, color: '#dc2626' };
      case 2:
        return { name: 'Teacher', icon: Briefcase, color: '#2563eb' };
      case 3:
        return { name: 'Student', icon: GraduationCap, color: '#059669' };
      case 4:
        return { name: 'Parent', icon: Users, color: '#d97706' };
      default:
          ...
          <div className={styles.profileSection}>
            ...
            <div className={styles.profileGrid}>
              <div className={styles.profileField}>
                ...
                <span>
                  {(user as TeacherData).employeeId || 'Not provided'}
                </span>
              </div>
              <div className={styles.profileField}>
                <label>Experience</label>
                <span>
                  {(user as TeacherData).experience
                    ? `${(user as TeacherData).experience} years`
                    : 'Not provided'}
                </span>
              </div>
              <div className={styles.profileField}>
                ...
                <span>
                  {(user as TeacherData).DOJ
                    ? new Date((user as TeacherData).DOJ).toLocaleDateString()
                    : 'Not provided'}
                </span>
              </div>
            </div>
