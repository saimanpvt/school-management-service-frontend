...
const AdminAttendance = () => {
    ...
    const [filterStatus, setFilterStatus] = useState<'all' | 'present' | 'absent' | 'low'>('all');
    ...
    const fetchAttendanceData = useCallback(async () => {
        ...
        try {
            ...
            if (studentsResponse.success) {
                ...
                const studentsWithAttendance = studentsResponse.data.map((student: AttendanceRecord) => ({
                    ...student,
                    attendance: {
                        present: Math.floor(Math.random() * 80) + 120,
                        total: Math.floor(Math.random() * 20) + 150,
                        percentage: Math.floor(Math.random() * 30) + 70,
                        lastPresent: new Date(Date.now() - Math.floor(Math.random() * 7) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            ...
            if (teachersResponse.success) {
                ...
                const teachersWithAttendance = teachersResponse.data.map((teacher: AttendanceRecord) => ({
                    ...teacher,
                    attendance: {
                        present: Math.floor(Math.random() * 80) + 120,
                        total: Math.floor(Math.random() * 20) + 150,
    ...
    const getAttendanceStats = () => {
        ...
        return { totalUsers, presentToday, absentToday, lowAttendance, avgAttendance };
    };
    ...
    const handleMarkAttendance = (userId: string, status: 'present' | 'absent') => {
        ...
        updateData(prev => prev.map(record =>
            record._id === userId
                ? {
                    ...record,
                    recentAttendance: [
                ...
                <div className={styles.statsGrid}>
                    ...
                    <div className={styles.statCard}>
                        ...
                        <div className={styles.statInfo}>
                            ...
                            <p className={styles.statNumber}>{stats.avgAttendance.toFixed(1)}%</p>
                        </div>
                    </div>
                </div>
                ...
                <div className={styles.tabsContainer}>
                    ...
                    <div className={styles.tabContent}>
                        ...
                        <div className={styles.filtersRow}>
                            ...
                            <select
                                value={filterStatus}
                                onChange={(e) => setFilterStatus(e.target.value as 'all' | 'present' | 'absent' | 'low')}
                                        ...
                                        <tr key={record._id}>
                                            ...
                                            <td>
                                                <span
                                                    ...
                                                    style={{ color: getAttendanceColor(record.attendance.percentage) }}
                                                >
                                                    {record.attendance.percentage}%
                                                </span>
                                            </td>
                                            <td>{record.attendance.present}/{record.attendance.total}</td>
                                            <td>
                                            ...
                                            <td>
                                                <span
                                                    className={`${styles.statusBadge} ${styles[record.recentAttendance[0]?.status || 'unknown']}`}
                                                >
                                            ...
                                            <td>
                                                <div className={styles.actionButtons}>
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => handleMarkAttendance(record._id, 'absent')}
                                                        title="Mark Absent"
                                                    >
                                                        <XCircle size={16} />
                                                    </button>
