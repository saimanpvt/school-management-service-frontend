            ...
            <main className={styles.main}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                    <div style={{ background: 'white', padding: '1.5rem', borderRadius: '1rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                        <div style={{ color: '#64748b', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Total Paid</div>
                        <div style={{ fontSize: '2rem', fontWeight: 700, color: '#059669' }}>${totalPaid.toLocaleString()}</div>
                    </div>
                    <div style={{ background: 'white', padding: '1.5rem', borderRadius: '1rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                        <div style={{ color: '#64748b', fontSize: '0.875rem', marginBottom: '0.5rem' }}>Pending Payment</div>
                        <div style={{ fontSize: '2rem', fontWeight: 700, color: '#dc2626' }}>${totalPending.toLocaleString()}</div>
                    </div>
                </div>
                            ...
                            <div key={fee.id} className={styles.feeCard}>
                                <div className={styles.feeInfo}>
                                    ...
                                    <span style={{
                                        ...
                                        color: fee.status === 'paid' ? '#059669' : fee.status === 'overdue' ? '#dc2626' : '#d97706'
                                    }}>
                                        {fee.status === 'paid' && <CheckCircle size={12} style={{ marginRight: '0.25rem' }} />}
                                        {fee.status === 'overdue' && <AlertCircle size={12} style={{ marginRight: '0.25rem' }} />}
