...
const TeacherAttendance = () => {
    ...
    useEffect(() => {
        const loadStudents = async () => {
            if (selectedClass) {
                try {
                    const response = await apiServices.teacher.getStudentsByClass(selectedClass);
                    if (response.success && response.data) {
                ...
                {students.length > 0 ? (
                    <div className={styles.attendanceList}>
                        {students.map(student => {
                            const attendanceRecord = attendance.find(record => record.studentId === student.id);
                            return (
                                <div key={student.id} className={styles.studentCard}>
                                    ...
                                    <div className={styles.attendanceButtons}>
                                        <button
                                            className={`${styles.attendanceBtn} ${attendanceRecord?.status === 'present' ? styles.active : ''}`}
                                        ...
                                        <button
                                            className={`${styles.attendanceBtn} ${attendanceRecord?.status === 'absent' ? styles.active : ''}`}
