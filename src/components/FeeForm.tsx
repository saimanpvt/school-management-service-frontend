...
const FeeForm: React.FC<FeeFormProps> = ({
    ...
    const frequencyOptions = ['One-time', 'Monthly', 'Quarterly', 'Semester', 'Annual'];
    ...
    const handleComponentChange = (index: number, field: keyof FeeComponent, value: any) => {
        ...
        const totalAmount = updatedComponents.reduce((sum, component) => sum + component.amount, 0);
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formData.feeComponents.length === 0) {
            ...
            formData.feeComponents.forEach((component, index) => {
                ...
                if (!component.amount || component.amount <= 0) missing.push(`Fee Component ${index + 1} Amount`);
            });
        }
    ...
    return (
        <div className={styles.formOverlay}>
            <div className={`${styles.formContainer} ${styles.extraLargeForm}`}>
                ...
                <form onSubmit={handleFormSubmit} className={styles.form} style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                    ...
                    <div className={styles.formRow}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            />
                            <small style={{ color: '#64748b' }}>Auto-calculated from fee components</small>
                        </div>
                    </div>
                    ...
                    <div style={{ marginTop: '2rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                            <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '600' }}>Fee Components</h3>
                            <button
                                type="button"
                        ...
                        {formData.feeComponents.map((component, index) => (
                            <div key={index} className={styles.componentCard} style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                    <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: '500' }}>Component {index + 1}</h4>
                                    <button
                                        type="button"
                                ...
                                <div className={styles.formRow}>
                                    <div className={styles.formGroup}>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleComponentChange(index, 'dueDate', e.target.value)}
                                        />
                                    </div>
                                    <div className={styles.formGroup}>
                                    ...
                                    <div className={styles.checkboxGroup}>
                                        <label>
                                            <input
                                                ...
                                                onChange={(e) => handleComponentChange(index, 'isRecurring', e.target.checked)}
                                            />
                                            Recurring
                                        </label>
                                    </div>
                        ...
                        {formData.feeComponents.length === 0 && (
                            <p style={{ color: '#64748b', textAlign: 'center', padding: '2rem' }}>
                                No fee components added. Click "Add Component" to add fee components.
                            </p>
                        )}
                    </div>
